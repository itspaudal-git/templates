...
case "$EXTENSION" in
  js|jsx|ts|tsx|mjs|cjs)
    if [ -f "$ROOT/node_modules/.bin/vitest" ]; then
      OUTPUT=$(cd "$ROOT" && pnpm exec vitest run "$REL_TEST" 2>&1); EXIT=$?
    elif [ -f "$ROOT/node_modules/.bin/jest" ]; then
      OUTPUT=$(cd "$ROOT" && pnpm exec jest "$REL_TEST" 2>&1); EXIT=$?
    elif [ -f "$ROOT/node_modules/.bin/mocha" ]; then
      OUTPUT=$(cd "$ROOT" && pnpm exec mocha "$REL_TEST" 2>&1); EXIT=$?
    else
      OUTPUT=$(cd "$ROOT" && pnpm test -- "$REL_TEST" 2>&1); EXIT=$?
    fi
    ;;
  py)
